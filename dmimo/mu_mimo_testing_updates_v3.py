...
def sim_mu_mimo(
    ...
    ns3cfg: Ns3Config,
    rc_config: RCConfig,
    rl_selector: Optional[RLTxUESelector] = None,
):
    """
    ...
    if not cfg.scheduling:
        rx_ue_mask = np.zeros(ns3cfg.num_rxue)
        tx_ue_mask = np.zeros(ns3cfg.num_txue)
        rx_ue_mask[:tmp_num_rxue_sel] = 1

        selected_tx_indices = None
        if rl_selector is not None:
            selected_tx_indices = rl_selector.prepare_tx_ue_selection(
                num_txues=ns3cfg.num_txue,
                num_txue_sel=tmp_num_txue_sel,
                last_tx_mask=getattr(cfg, "last_tx_ue_mask", None),
                mcs_indices=getattr(cfg, "last_mcs_indices", None),
                node_wise_acks=getattr(cfg, "last_node_wise_acks", None),
                throughput_debug=getattr(cfg, "last_throughput", None),
                drop_idx_debug=cfg.drop_idx,
            )

        if selected_tx_indices is None:
            selected_tx_indices = list(range(tmp_num_txue_sel))

        if len(selected_tx_indices) < tmp_num_txue_sel:
            for candidate in range(ns3cfg.num_txue):
                if candidate in selected_tx_indices:
                    continue
                selected_tx_indices.append(candidate)
                if len(selected_tx_indices) >= tmp_num_txue_sel:
                    break

        selected_tx_indices = selected_tx_indices[:tmp_num_txue_sel]
        tx_ue_mask[selected_tx_indices] = 1
        cfg.last_tx_ue_mask = tx_ue_mask.copy()
        ...
        ue_indices = [[0, 1], [2, 3]]  # Assuming gNB was scheduled
        scheduled_tx_UEs = np.array(selected_tx_indices, dtype=int) + 1
...
def sim_mu_mimo_all(
    ...
    for first_slot_idx in np.arange(cfg.start_slot_idx, cfg.total_slots, cfg.num_slots_p1 + cfg.num_slots_p2):
        ...
        start_time = time.time()
        bers, bits, additional_KPIs = sim_mu_mimo(cfg, ns3cfg, rc_config, rl_selector=rl_selector)
        end_time = time.time()
