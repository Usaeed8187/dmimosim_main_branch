...
class ESN():
    ...
    def fit(self, inputs, outputs, nForgetPoints):
        ...
        outputs_pred = np.zeros((inputs.shape[0] + 1, self.n_outputs))
        error = np.zeros((inputs.shape[0] + 1, self.n_outputs))

        for n in range(inputs.shape[0]):
            states[n + 1, :] = self._update(states[n], inputs_scaled[n + 1, :], teachers_scaled[n, :])
            extended_states[n + 1, :] = np.hstack((states[n + 1, :], inputs_scaled[n + 1, :]))
            outputs_pred[n + 1, :] = np.dot(self.W_out, extended_states[n + 1, :].T)
            error[n + 1, :] = outputs_pred[n + 1, :] - teachers_scaled[n + 1, :]
            if (n >= nForgetPoints):
                regu = np.sum(outputs_pred[n, :]**2)
                regu = regu + 0.002
                self.W_out = self.W_out - self.lr / regu * \
                             np.dot(error[n + 1, :].reshape((-1, 1)), extended_states[n, :].reshape((1, -1)))
