...
class ESN():
    ...
    def __init__(self, n_inputs, n_outputs, n_reservoir, n_layers,
    ...
    def _initweights(self):
        ...
        self.W_out = np.random.uniform(-1, 1, size=(self.n_outputs, self.n_reservoir * self.n_layers + int(self.n_inputs/2)))

    def _update(self, state, input, output, a=0.7):
    ...
    def fit(self, inputs, outputs, nForgetPoints):
        ...
        states_train = np.zeros((len(states_train_list), self.n_reservoir * self.n_layers + int(self.n_inputs/2)))
        states_train[: , :int(self.n_inputs/2)] = inputs_scaled[nForgetPoints + 1:, int(self.n_inputs/2):]
        for i in range(self.n_layers):
            for n in range(len(states_train_list)):
                states_train[n, int(self.n_inputs/2) + i * self.n_reservoir : int(self.n_inputs/2) + (i+1) * self.n_reservoir] = states_train_list[n][i]
        ...
        gradient[gradient / error_train.shape[1] > 0.1] = 0.1
        gradient[gradient / error_train.shape[1] < -0.1] = -0.1
        #self.W_out = self.W_out - self.lr * gradient / np.linalg.norm(gradient)
    ...
    def predict(self, inputs, nForgetPoints, continuation=True):
        ...
        for n in range(n_samples):
            ...
            extended_states = np.zeros(self.n_reservoir * self.n_layers + int(self.n_inputs/2))
            extended_states[: int(self.n_inputs/2)] = inputs[n + 1, int(self.n_inputs/2):]
            for i in range(self.n_layers):
                extended_states[int(self.n_inputs/2) + i * self.n_reservoir : int(self.n_inputs/2) + (i+1) * self.n_reservoir] = states[n+1][i]

            outputs[n + 1, :] = self.out_activation(np.dot(self.W_out, extended_states))
