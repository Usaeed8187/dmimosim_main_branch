...
if __name__ == "__main__":
    ...
    for random_seed in range(11,12):
        ...
        dim_states = n_channel + 1 + 2 # The sensing result space
        ...
        observation_input = np.zeros((n_su, dim_states))
        observation_input[:, :dim_states - 2] = observation[:, :]
        ...
        for step in range(total_episode):
            ...
            for k in range(n_su):
                ...
                DRQN_list[k]
                observation_input[k, :]
                action[k] = DRQN_list[k].choose_action(observation_input[k, :])
            ...
            if (step+1) < total_episode:
                ...
                observation_input_ = np.zeros((n_su, dim_states))
                for k in range(n_su):
                    observation_input_[k, :dim_states - 2] = observation_[k, :]
                    if reward_SU[k, step] == 0:
                        observation_input_[k, -1] = 1
                    else:
                        observation_input_[k, -2] = 1
        ...
        print('Training Time = %.4f sec' % training_time)
