...
class ESN():
    ...
    def _initweights(self):
        ...
        for i in range(self.n_layers):
            ...
            W_tmp = np.random.uniform(-0.5, 0.5, size=(self.n_reservoir, self.n_reservoir))
            # delete the fraction of connections given by (self.sparsity):
            W_tmp[np.random.uniform(0, 1, size=(W_tmp.shape[0], W_tmp.shape[1])) < self.sparsity] = 0
        ...
        for i in range(self.n_layers):
            if (i == 0):
                #self.W_in.append(np.random.uniform(-1, 1, size=(self.n_reservoir, int(self.n_inputs/2))))
                self.W_in.append(np.random.uniform(-1, 1, size=(self.n_reservoir, self.n_inputs)))
            else:
                self.W_in.append(np.random.uniform(-1, 1, size=(self.n_reservoir, self.n_reservoir)))

        # output MLP:
        MLP_neurons = self.n_reservoir * self.n_layers + self.n_inputs
        self.tf_input = tf.placeholder(tf.float32, [None, self.n_reservoir * self.n_layers + self.n_inputs])
        self.tf_target = tf.placeholder(tf.float32, [None, self.n_outputs])
        self.tf_lr = tf.placeholder(tf.float32, [])

        self.tf_w1 = tf.Variable(tf.random_normal([self.n_reservoir * self.n_layers + self.n_inputs,
                                                   MLP_neurons], mean=0.0, stddev=0.3))
        self.tf_b1 = tf.Variable(tf.random_normal([1, MLP_neurons], mean=0.0, stddev=0.3))
        self.tf_l1 = tf.nn.tanh(tf.matmul(self.tf_input, self.tf_w1) + self.tf_b1)

        self.tf_w2 = tf.Variable(tf.random_normal([MLP_neurons, self.n_outputs], mean=0.0, stddev=0.3))
        self.tf_b2 = tf.Variable(tf.random_normal([1, self.n_outputs], mean=0.0, stddev=0.3))
        self.tf_output = tf.matmul(self.tf_l1, self.tf_w2) + self.tf_b2

        self.tf_loss = tf.reduce_mean(tf.squared_difference(self.tf_target, self.tf_output))
        self.train_op = tf.train.AdamOptimizer(self.tf_lr).minimize(self.tf_loss)

    def _update(self, state, input, a=0.7):
    ...
    def fit(self, outputs, index, gradient_clip=0.3):
        """
        ...
        _ = self.sess.run(self.train_op, feed_dict={self.tf_lr: self.lr, self.tf_input: states_train,
                                                    self.tf_target: teachers_scaled_train})
    ...
    def predict(self, inputs, nForgetPoints, continuation=True):
        ...
        for n in range(n_samples):
            ...
            #self.extended_states[self.memory_counter, : int(self.n_inputs/2)] = inputs[n + 1, int(self.n_inputs/2):]
            ...
            tf_input = self.extended_states[self.memory_counter, :]
            tf_input = tf_input.reshape((1, -1))
            outputs[n + 1, :] = self.sess.run(self.tf_output, feed_dict={self.tf_input: tf_input})
    ...
    def predict_training(self, index):
        ...
        tf_input = self.extended_states[index, :]
        outputs = self.sess.run(self.tf_output, feed_dict={self.tf_input: tf_input})
    ...
    def copy_net(self, net):
        replace_target_op = [tf.assign(self.tf_w1, net.tf_w1), tf.assign(self.tf_w2, net.tf_w2),
                             tf.assign(self.tf_b1, net.tf_b1), tf.assign(self.tf_b2, net.tf_b2)]
