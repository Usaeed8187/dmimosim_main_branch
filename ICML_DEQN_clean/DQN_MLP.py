...
class MLP1:
    def __init__(
        ...
        self._build_net(self.nInternalUnits, random_seed)
    ...
    def _build_net(self, nInternalUnits, random_seed=1):
    ...
    def learn(self):
        ...
        replace_target_iter = 25  #25

        for i in range(training_iteration):

            index_train = np.random.randint(0, self.memory_size, training_batch_size)

            q_next, q_eval = self.sess.run(
                [self.q_next, self.q_eval],
                feed_dict={
                    self.s_: batch_memory[index_train, -self.n_features:],  # fixed params
                    self.s: batch_memory[index_train, :self.n_features],  # newest params
                })

            # change q_target w.r.t q_eval's action
            q_target = q_eval.copy()

            eval_act_index = batch_memory[index_train, self.n_features].astype(int)
            reward = batch_memory[index_train, self.n_features + 1]
            ...
            # train eval network
            _ = self.sess.run([self._train_op], feed_dict={self.s: batch_memory[index_train, :self.n_features], self.q_target: q_target})
