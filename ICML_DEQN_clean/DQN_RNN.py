...
class DRQN:
    def __init__(
        ...
        self.nInternalUnits = nInternalUnits  # 32
    ...
    def _build_net(self, su_index, random_seed=1):
        ...
        with tf.variable_scope('eval_net' + str(su_index)):
            self.rnnCell1_eval = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.nInternalUnits)
        ...
        with tf.variable_scope('eval_net_train' + str(su_index)):
            self.rnnCell1_eval_train = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.nInternalUnits)
        ...
        with tf.variable_scope('target_net' + str(su_index)):
            self.rnnCell1_target = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.nInternalUnits)
