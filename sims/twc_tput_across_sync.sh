...
# declare -a channel_prediction_settings=("configured_wesn_balanced_lite") # Phase-2 predictors; configured identically to the phase-2-only sweep.
declare -a csi_quantization_arr=("True")

# Residual synchronization sweep. Each entry is
# "frequency-error std. dev. (degrees), timing-error std. dev. (samples)".
# The zero-error baseline is generated by twc_tput_across_tx.sh.
                                ...
                                for cp_setting in ${!channel_prediction_settings[@]}; do
                                    for cquant in ${!csi_quantization_arr[@]}; do
                                        ...
                                        for sync_error_pair in "${sync_error_pairs[@]}"; do
                                            read -r phase_std_deg timing_std_samples <<< "${sync_error_pair}"
                                            sync_errors="True"
