...
class ESN():
    ...
    def __init__(self, n_inputs, n_outputs, n_reservoir=64, n_layers=2,
    ...
    def _initweights(self):
        ...
        for i in range(self.n_layers):
            if (i == 0):
                ...
                self.W_in.append(self.random_state_.rand(self.n_reservoir, self.n_reservoir) * 2 - 1)

        # random output weights:
        #self.W_out = self.random_state_.rand(self.n_outputs, self.n_reservoir * self.n_layers + self.n_inputs) * 2 - 1
        self.W_out = self.random_state_.rand(self.n_outputs, self.n_reservoir * self.n_layers)
    ...
    def fit(self, inputs, outputs, nForgetPoints):
        ...
        states_train = np.zeros((len(states_train_list), self.n_reservoir * self.n_layers))
        #states_train = np.zeros((len(states_train_list), self.n_reservoir * self.n_layers + self.n_inputs))
        #states_train[: , :self.n_inputs] = inputs_scaled[nForgetPoints + 1:, :]
        for i in range(self.n_layers):
            for n in range(len(states_train_list)):
                #states_train[n, self.n_inputs + i * self.n_reservoir : self.n_inputs + (i+1) * self.n_reservoir] = states_train_list[n][i]
                states_train[n, i * self.n_reservoir : (i + 1) * self.n_reservoir] = states_train_list[n][i]
    ...
    def predict(self, inputs, nForgetPoints, continuation=True):
        ...
        for n in range(n_samples):
            ...
            extended_states = np.zeros(self.n_reservoir * self.n_layers)
            #extended_states = np.zeros(self.n_reservoir * self.n_layers + self.n_inputs)
            #extended_states[: self.n_inputs] = inputs[n + 1, :]
            for i in range(self.n_layers):
                #extended_states[self.n_inputs + i * self.n_reservoir : self.n_inputs + (i+1) * self.n_reservoir] = states[n+1][i]
                extended_states[i * self.n_reservoir : (i + 1) * self.n_reservoir] = states[n + 1][i]
