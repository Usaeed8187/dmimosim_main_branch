...
class Baseline(Model):
    ...
    def call(self, dmimo_chans: dMIMOChannels, h_freq_csi, info_bits, precoding_matrices=None):
        """
...
def do_rank_link_adaptation(cfg, dmimo_chans, h_est, rx_snr_db):
    ...
    link_adaptation = linkAdaptation(dmimo_chans.ns3_config.num_bs_ant, dmimo_chans.ns3_config.num_ue_ant,
                                     ...
                                     N_s=rank_feedback_report[0], data_sym_position=data_sym_position, lookup_table_size='long')
...
def sim_baseline(cfg: SimConfig, ns3cfg: Ns3Config):
    ...
    if cfg.rank_adapt and cfg.link_adapt:
        # Update total number of streams
        cfg.num_tx_streams = rank_feedback_report[0]

        print("\n", "rank (baseline) = ", cfg.num_tx_streams, "\n")
    ...
    if cfg.CSI_feedback_method =='5G':
        generate_CSI_feedback = quantized_CSI_feedback(method='5G', num_tx_streams=cfg.num_tx_streams, architecture='baseline', 
                                                        snrdb=rx_snr_db)
    ...
    dec_bits, uncoded_ber, uncoded_ser, x_hat = baseline(dmimo_chans, h_freq_csi, info_bits, precoding_matrices)
    ...
    return [uncoded_ber, coded_ber], [goodbits, userbits, ratedbits], [rank_out]
...
def sim_baseline_all(cfg: SimConfig, ns3cfg: Ns3Config):
    ...
    for first_slot_idx in np.arange(cfg.start_slot_idx, cfg.total_slots, cfg.num_slots_p2):
        ...
        bers, bits, additional_KPIs = sim_baseline(cfg, ns3cfg)
        uncoded_ber += bers[0]
