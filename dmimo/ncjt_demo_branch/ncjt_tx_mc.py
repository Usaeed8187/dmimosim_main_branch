...
class MC_NCJT_TxUE(Model):
    ...
    def call(self, bit_stream_dmimo:tf.Tensor, is_txbs:bool=False, cluster_idx:int = 0):
        ...
        """
        num_effective_subcarriers = int(self.cfg.fft_size - sum(self.cfg.num_guard_carriers))-int(self.cfg.dc_null)
        assert bit_stream_dmimo.shape[-1] == (self.cfg.symbols_per_slot - len(self.cfg.pilot_indices))*(self.modulation_orders[cluster_idx]) and \
            (bit_stream_dmimo.shape[-2]==num_effective_subcarriers) , \
