...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        # train eval network
        n_total = eval_net_input.shape[0]
        cost_min = float("inf")
        for k in range(5):
            tmp_eval_net = copy.deepcopy(self.eval_net)
            tmp_index_start = np.random.randint(100)
            tmp_index = np.arange(tmp_index_start, tmp_index_start+1900, dtype=int)
            #tmp_index = np.arange(n_total/4*k, n_total/4*(k+1), dtype=int)
            pred_train = tmp_eval_net.fit(eval_net_input[tmp_index], q_target[tmp_index], self.nForgetPoints)
            tmp_cost = np.linalg.norm(pred_train - q_target[tmp_index])
            if (tmp_cost < cost_min):
                cost_min = tmp_cost
                eval_net_minCost = copy.deepcopy(tmp_eval_net)
        self.eval_net = copy.deepcopy(eval_net_minCost)

        self.cost_his.append(cost_min)
