...
class ESN():
    ...
    def fit(self, inputs, outputs, nForgetPoints):
        ...
        extended_states_train = np.hstack((states[nForgetPoints + 1:, :], inputs_scaled[nForgetPoints + 1:, :]))
        teachers_scaled_train = teachers_scaled[nForgetPoints + 1:, :]
        error_train = np.dot(self.W_out, extended_states_train.T) - teachers_scaled_train.T
        self.W_out = self.W_out - self.lr * np.dot(error_train, extended_states_train) / error_train.shape[1]
