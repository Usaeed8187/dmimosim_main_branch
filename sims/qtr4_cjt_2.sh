...
declare -a phase_1_enabled_arr=("True")
                            ...
                            for pcsi in ${!perfect_csi_arr[@]}; do
                                for cp_setting in ${!channel_prediction_settings[@]}; do
                                    for cquant in ${!csi_quantization_arr[@]}; do
                                        for p1 in ${!phase_1_enabled_arr[@]}; do
...
run_scenario() {
    local args=("$@")
    python sims/sim_mu_mimo_testing_updates_w_p1.py "${args[@]}"
}
