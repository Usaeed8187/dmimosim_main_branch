...
class ESN():
    ...
    def _initweights(self):
        ...
        for i in range(self.n_layers):
            if (i == 0):
                ...
                self.W_in.append(np.random.uniform(-1, 1, size=(self.n_reservoir, self.n_outputs)))

        # random output weights:
        self.W_out = []
        for i in range(self.n_layers):
            self.W_out.append(np.random.uniform(-1, 1, size=(self.n_outputs, self.n_reservoir)))
    ...
    def _update(self, state, input, a=0.7):
        ...
        for i in range(self.n_layers):
            ...
            if (i < self.n_layers - 1):
                tmp_input = np.dot(self.W_out[i], state_new[i])
    ...
    def fit(self, inputs, outputs, nForgetPoints):
        ...
        inputs_scaled = np.vstack(
            [self.startinput, self._scale_inputs(inputs)])
        teachers_scaled = np.vstack(
            [self.startoutput, self._scale_teacher(outputs)])
        for n in range(n_samples):
            states.append(self._update(states[n], inputs_scaled[n + 1, :]))
    ...
    def predict(self, inputs, nForgetPoints, continuation=True):
        ...
        for n in range(n_samples):
            ...
            outputs[n + 1, :] = self.out_activation(np.dot(self.W_out[self.n_layers-1], states[n+1][self.n_layers-1]))
    ...
    def refresh_laststate(self):
        self.laststate = copy.deepcopy(self.startstate)
        self.lastinput = copy.deepcopy(self.startinput)
        self.lastoutput = copy.deepcopy(self.lastoutput)
