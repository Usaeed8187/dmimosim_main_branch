...
class DRQN:
    ...
    def _build_net(self, su_index):
        ...
        with tf.variable_scope('eval_net' + str(su_index)):
            self.rnnCell1_eval = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell1')
            ...
            self.rnnCell2_eval = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell2')
            self.init_s2_eval = self.rnnCell2_eval.zero_state(batch_size=1, dtype=tf.float32)
            self.outs3D2_eval, self.hidden_s2_eval = tf.nn.dynamic_rnn(self.rnnCell2_eval, self.outs3D1_eval,
                                                                       initial_state=self.init_s2_eval,
                                                                       time_major=False)

            self.outs2D2_eval = tf.reshape(self.outs3D2_eval, [-1, nInternalUnits])
            self.netOuts2D_eval = tf.layers.dense(self.outs2D2_eval, self.n_actions)

            self.q_eval = tf.reshape(self.netOuts2D_eval, [-1, 1, self.n_actions])

        with tf.variable_scope('eval_net_train' + str(su_index)):
            self.rnnCell1_eval_train = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell1')
            ...
            self.rnnCell2_eval_train = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell2')
            self.init_s2_eval_train = self.rnnCell2_eval_train.zero_state(batch_size=1, dtype=tf.float32)
            self.outs3D2_eval_train, self.hidden_s2_eval_train = tf.nn.dynamic_rnn(self.rnnCell2_eval_train,
                                                                                   self.outs3D1_eval_train,
                                                                                   initial_state=self.init_s2_eval_train,
                                                                                   time_major=False)

            self.outs2D2_eval_train = tf.reshape(self.outs3D2_eval_train, [-1, nInternalUnits])
            self.netOuts2D_eval_train = tf.layers.dense(self.outs2D2_eval_train, self.n_actions)
        ...
        with tf.variable_scope('target_net' + str(su_index)):
            self.rnnCell1_target = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell1')
            ...
            self.rnnCell2_target = tf.nn.rnn_cell.LSTMCell(num_units=nInternalUnits, name='rnncell2')
            self.init_s2_target = self.rnnCell2_target.zero_state(batch_size=1, dtype=tf.float32)
            self.outs3D2_target, self.hidden_s2_target = tf.nn.dynamic_rnn(self.rnnCell2_target, self.outs3D1_target,
                                                                           initial_state=self.init_s2_target,
                                                                           time_major=False)

            self.outs2D2_target = tf.reshape(self.outs3D2_target, [-1, nInternalUnits])
            self.netOuts2D_target = tf.layers.dense(self.outs2D2_target, self.n_actions)
    ...
    def choose_action(self, observation):
        ...
        if (self.final_s1_eval == [] and self.final_s2_eval == []):
            actions_value, self.final_s1_eval, self.final_s2_eval = \
                self.sess.run([self.q_eval, self.hidden_s1_eval, self.hidden_s2_eval],
            ...
            actions_value, self.final_s1_eval, self.final_s2_eval = \
                self.sess.run([self.q_eval, self.hidden_s1_eval, self.hidden_s2_eval],
                              feed_dict={self.s: observation,
                                         ...
                                         self.init_s2_eval: self.final_s2_eval})
