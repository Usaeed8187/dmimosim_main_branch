...
class ESN():
    ...
    def _initweights(self, random_seed):
        ...
        if self.W_out_type == 1:
            self.W_out = np.random.uniform(-1, 1, size=(self.n_outputs, self.n_reservoir * self.n_layers + self.n_inputs))
        else:
            self.W_out = np.random.uniform(-1, 1, size=(self.n_outputs, self.n_reservoir * self.n_layers))
        r = np.max(np.abs(np.linalg.norm(self.W_out, 2)))
        self.W_out = self.W_out * (1 / r)

    def _update(self, state, input, a=1):
        ...
        for n in range(n_samples):
            ...
            for i in range(self.n_layers):
                ...
                if self.W_out_type == 1:
                    ...
                    self.extended_states[self.memory_counter,
                    i * self.n_reservoir:
                    (i + 1) * self.n_reservoir] = states[n + 1][i]
