...
from DQN_basicRNN import basicDRQN
import numpy as np
import copy
import dill
import time
...
if __name__ == "__main__":
    ...
    # Number of channel, Number of PUs per channel, Number of SUs
        ...
        DRQN_list = []
        for k in range(n_su):
            DRQN_tmp = basicDRQN(k, dim_actions, dim_states,
                            reward_decay=0.9,
        ...
        for step in range(total_episode):
            ...
            if ((step + 1) % (batch_size) == 0):
                ...
                if save_norms:
                    for k in range(n_su):
                        W_rec = DRQN_list[k].sess.run(DRQN_list[k].rnnCell1_eval.trainable_variables[0][-DRQN_list[k].nInternalUnits:,:])
                        Brec_SU_2[k,step+1-batch_size:step] = np.linalg.norm(W_rec,ord=2)
                        Brec_SU_F[k,step+1-batch_size:step] = np.linalg.norm(W_rec,ord='fro')
                        W_in = DRQN_list[k].sess.run(DRQN_list[k].rnnCell1_eval.trainable_variables[0][:-DRQN_list[k].nInternalUnits,:])
                        Bin_SU_2[k,step+1-batch_size:step] = np.linalg.norm(W_in,ord=2)
                        Bin_SU_F[k,step+1-batch_size:step] = np.linalg.norm(W_in,ord='fro')
                        W_out = DRQN_list[k].sess.run(DRQN_list[k].eval_output_weights)
                        Bout_SU_2[k,step+1-batch_size:step] = np.linalg.norm(W_out,ord=2)
                        Bout_SU_F[k,step+1-batch_size:step] = np.linalg.norm(W_out,ord='fro')
