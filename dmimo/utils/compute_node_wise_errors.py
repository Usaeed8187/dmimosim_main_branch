...
def compute_UE_wise_BER(info_bits, dec_bits, num_tx_streams_per_node, num_tx_streams):
    ...
    num_rx_UEs = num_tx_streams // num_tx_streams_per_node
    ...
    node_wise_ber = 0.5*np.ones(num_rx_UEs)
    node_wise_bler = 0.5*np.ones(num_rx_UEs)
    for node_idx in range(num_rx_UEs):

        node_wise_ber[node_idx] = compute_ber(tf.gather(info_bits, UE_indices[node_idx], axis=2), tf.gather(dec_bits, UE_indices[node_idx], axis=2)).numpy()
        node_wise_bler[node_idx] = compute_bler(tf.gather(info_bits, UE_indices[node_idx], axis=2), tf.gather(dec_bits, UE_indices[node_idx], axis=2)).numpy()
